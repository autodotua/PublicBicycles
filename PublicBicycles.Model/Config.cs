...
namespace PublicBicycles.Models
{
    public class Config : IDbModel
    {
        ...
        public async static Task<string> GetAsync(PublicBicyclesContext db, string key, string defaultValue)
        {
            string value = (await db.Configs.FirstOrDefaultAsync(p => p.Key == key))?.Value;
            return value ?? defaultValue;
        }
        public async static Task SetAsync(PublicBicyclesContext db, string key, string value)
        {
