<template>
  <div>
    ...
    <el-card class="box-card">
      ...
      <div v-for="publicBicycles in publicBicycless" :key="publicBicycles.id">
        <h4>{{publicBicycles.name}}</h4>
        <a :href="getImageUrl(publicBicycles.id)" style="width:100%">
          <img :src="getImageUrl(publicBicycles.id)" style="width:100%" />
        </a>
      </div>
    </el-card>
  </div>
    ...
    this.$nextTick(function() {
      ...
      Vue.axios
        ...
        .then(response => {
          ...
          this.publicBicycless = response.data.data.publicBicycless;
          console.log(this.publicBicycless);
        })
        .catch(showError);
    });
  }
