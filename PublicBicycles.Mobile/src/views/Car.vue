<template>
  ...
  <div>
    ...
    <el-drawer title="停车记录" :visible.sync="drawerDetail" :with-header="true" size="360px">
      <el-table :data="publicBicyclesRecords" style="width: 100%">
        ...
        <el-table-column prop="publicBicyclesArea.name" label="停车场" width="80"></el-table-column>
      </el-table>
    </el-drawer>
  ...
  methods: {
    viewDetail(row: any) {
      ...
      Vue.axios
        ...
        .then(response => {
          for (const record of response.data.data.publicBicyclesRecords) {
            record.enterTime = formatDateTime(record.enterTime as  string);
            record.leaveTime = formatDateTime(record.leaveTime as  string);
          }
          this.publicBicyclesRecords = response.data.data.publicBicyclesRecords;
        })
        .catch(showError);
    },
    deleteCar(row: any) {
